...
function UserProfilePage() {
	...
	useEffect(() => {
		...
		async function checkFollowing() {
			const docRef = doc(db, `users/${currentUser.userid}/following`, userid);
			const docUserSnap = await getDoc(docRef);
	...
	useEffect(() => {
		if(userid){onSnapshot(query(collection(db, `posts`), where("userRef", "==", doc(db, "users", userid))), (snapshot) => {
			setPosts(snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id })));
		});}
	}, [userid]);

	async function followUser() {
		await setDoc(doc(db, `users/${currentUser.userid}/following`, user.id), {});
		await setDoc(doc(db, `users/${user.id}/followers`, currentUser.userid), {});
		setIsFollowing(true);
		setFollowersCount(followersCount + 1);
	}

	async function unfollowUser() {
		await deleteDoc(doc(db, `users/${currentUser.userid}/following`, user.id), {});
		await deleteDoc(doc(db, `users/${user.id}/followers`, currentUser.userid), {});
